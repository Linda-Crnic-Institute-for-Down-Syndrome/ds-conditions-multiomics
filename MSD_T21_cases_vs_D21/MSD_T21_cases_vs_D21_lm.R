################################################
# Title: Differential abundance analysis of plasma MSD cytokines and immune
# markers in T21 cases per condition vs. D21
# Author(s):
#   - Srija Chillamcherla
#   - Matthew Galbraith
# affiliation(s):
...
# ELISA from individuals with (T21) and without (D21) Down syndrome.
...
out_file_prefix <- "MSD_T21_cases_vs_D21_lm.R_" # should match this script title
...
regression_prep_dat <- msd %>% 
  ...
  mutate(
    ...
    Karyotype = fct_relvel(Karyotype, c("Control", "T21")) # ensure factor levels in correct order
  ) %>% 
  group_by(Karyotype, Analyte, condition) %>% 
...
#
regression_prep_dat_final %>% 
  distinct(LabID) # 467 individuals are being analyzed here
#
...
v_plot <- all_cond_analytes_lm_data_final_results %>%
  ...
  volcano_plot_lab_lm(
    title="Diff. abundance of MSD cytokines in Obesity (T21 cases vs. D21) - linear regression",
