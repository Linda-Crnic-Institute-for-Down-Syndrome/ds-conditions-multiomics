################################################
# Title: Differential abundance analysis of immune cell populations in T21 controls per condition vs. D21
# Author(s):
#   - Srija Chillamcherla
#   - Matthew Galbraith
# affiliation(s):
...
# measured by the CyTOF FlowSOMv2 from individuals with (T21) and without (D21) Down syndrome.
...
out_file_prefix <- "CyTOF_T21_controls_vs_D21_bm.R_" # should match this script title
...
#
# inspect
cytof %>% distinct(LabID) 
cytof %>% distinct(cluster)
# 
...
regression_prep_dat <- cytof %>% 
  ...
  # Outlier removal
  group_by(uniqueID, condition, Karyotype) %>%  
  mutate(
...
for (cond in conditions$condition) {
  ...
  betareg_data_final.tmp <- NULL # initializing
  betareg_data_final.tmp <- regression_prep_dat_final %>%
    filter(condition == cond) %>%
    select(LabID, uniqueID, cluster, proportion_Live, proportion_subset, Sex, Age, Karyotype, Sample_source_code, condition, has_cond) %>%
    nest(data = c(LabID, proportion_subset, Sex, Age, Karyotype, Sample_source_code, has_cond))
  
  # Debug: Check the condition data
  if (nrow(betareg_data_final.tmp) == 0) {
  ...
  regressions_multi_cytof_AgeSexSource.tmp <- betareg_data_final.tmp %>%
    mutate(
      fit = map(data, ~ betareg(proportion_subset ~ Karyotype + Sex + Age + Sample_source_code, data = .x, link = "logit")),
  ...
  betareg_cytof_multi_ageSexSource_results.tmp <- regressions_multi_cytof_AgeSexSource.tmp %>%
    ...
    mutate(Model = "betareg(proportion ~ Karyotype + Sex + Age + Source)")
...
v_plot <- all_cond_cytof_betareg_data_final_results %>%
  ...
  volcano_plot_lab_lm(
    title= "Diff. abundance of cells in Hidradenitis suppurativa (T21 controls vs. D21) - beta regression",
...
betareg_data_final_adj %>% 
  ...
  ggroup_by(uniqueID, condition, Karyotype) %>%  
  mutate(
    logit_proportion_subset = gtools::logit(proportion_subset),
    extreme_proportion_subset = rstatix::is_extreme(logit_proportion_subset),
  ) %>% 
  filter(extreme_proportion_subset == FALSE) %>% 
  ungroup() %>% 
  # re leveling the karyotype 
  mutate(Karyotype = fct_relevel(Karyotype, c("Control", "T21"))) %>% 
  ggplot(aes(x = Karyotype,  y = gtools::logit(proportion_subset), color = Karyotype)) +
