################################################################################
# Title: Differential abundance analysis of D21 plasma metabolites vs. BMI
# Author(s):
...
# metabolites in individuals without Down syndrome (D21).
...
#
out_file_prefix <- "Metabolomics_D21_BMI_lm.R_" # should match this script title
...
regressions_data <- metabolomics_data_meta_data %>% # 70,470 rows
  filter(Karyotype == "Control") |> # keeps 15,834 rows
  ...
  ungroup() %>% 
  filter(extreme_value != TRUE) %>% # keeps 15,796 rows (extreme outliers are removed per feature)
  # check here for:
...
lm_D21_Analyte_BMI_Age_Sex_Sample_source_results <- regressions_multi_AgeSexSourceFixed |> 
  unnest(tidied) |> 
...
#
lm_D21_Analyte_BMI_Age_Sex_Sample_source_results %>% 
...
lm_D21_Analyte_BMI_Age_Sex_Sample_source_results %>%
write_tsv(file = here("results","HTP D21 bmi plasma metabolome linear regressions results.txt"))
#


# 4 Plots ----
## 4.1 Volcano plots -----
v_plot <- lm_D21_Analyte_BMI_Age_Sex_Sample_source_results %>% 
  volcano_plot_lab_lm(
    title = "Diff. abundance of plasma metabolites (controls vs. BMI) - linear regression",
...
metabolomics_SourceSexAge_adj %>% 
  ...
  facet_wrap(~ Analyte , scales = "free_y", nrow = 1) +# facet per feature; each feature on it's own scale
  theme(aspect.ratio = 1) + # set fixed aspect ratio
  labs(title = "Top significant metabolites by fold-change: Control vs. BMI",
       subtitle = paste("Significant associations\nAgeSexSource adj\nextreme outliers are removed"),
       y = "log2_adj_val",
       x = "log2_adj_bmi"
       )
# save the plot
ggsave(filename = here("plots", paste0(out_file_prefix, "scatter_D21_top_signif_by_FC", ".pdf")), device = cairo_pdf, width = 15, height = 5, units = "in")
#
