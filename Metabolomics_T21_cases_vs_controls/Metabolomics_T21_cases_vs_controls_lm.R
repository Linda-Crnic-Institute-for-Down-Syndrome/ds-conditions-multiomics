################################################
# Title: Differential abundance analysis of plasma metabolites by co-occurring condition status (cases vs. controls) in individuals with Down syndrome (T21)
# Author(s):
#   - Srija Chillamcherla
#   - Matthew Galbraith
# affiliation(s):
...
out_file_prefix <- "Metabolomics_T21_cases_vs_controls_lm.R_" # should match this script title
...
regression_prep_dat <- metab %>% 
  inner_join(htp_cooccurring_data %>% filter(!is.na(has_cond))) %>% # keeps 10981836 rows
  filter(Karyotype == "T21") %>% # keeps 8396718 rows
  mutate(
  ...
  ungroup() %>% 
  filter(extrem_analyte_conc != TRUE) # keeps 8358038 rows
#
...
#
regression_prep_dat_final %>% 
  distinct(LabID) # 316 individuals are being analyzed here
#
...
v_plot <- all_cond_analytes_lm_data_final_results %>%
  ...
  volcano_plot_lab_lm(
    title="Diff. abundance of plasma metabolites in GERD (cases vs. controls) - linear regression",
