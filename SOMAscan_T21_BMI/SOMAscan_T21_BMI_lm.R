################################################################################
# Title: Differential abundance analysis of T21 plasma proteins vs. BMI
# Author(s):
...
# measured by the SOMAscan assay in individuals with Down syndrome.
...
#
out_file_prefix <- "SOMAscan_T21_BMI_lm.R_" # should match this script title
...
somascan_data %>% distinct(Aptamer) # 4632 measurements each for 419 samples = 1,940,808
#
...
regressions_data <- somascan_data_meta_data %>% #1,875,960 rows
  filter(Karyotype == "T21") |>  # keeps 1,454,448 rows
  ...
  ungroup() %>%
  filter(extreme_value != TRUE) %>% # keeps 1,425,828 rows (extreme outliers are removed per feature)
  # check here for:
...
lm_T21_Aptamer_BMI_Age_Sex_Sample_source_results <- regressions_multi_AgeSexSourceFixed |> 
  unnest(tidied) |> 
...
#
lm_T21_Aptamer_BMI_Age_Sex_Sample_source_results %>% 
  filter(BHadj_pval < 0.1 )
#

## 3.3 Export the results ----
lm_T21_Aptamer_BMI_Age_Sex_Sample_source_results %>%
  write_tsv(file = here("results","HTP T21 bmi plasma proteome linear regressions results.txt"))
#


# 4 Plots ----
## 4.1 Volcano plots -----
v_plot <- lm_T21_Aptamer_BMI_Age_Sex_Sample_source_results %>% 
  volcano_plot_lab_lm(
    title = "Diff. abundance of plasma proteins (T21 vs. BMI) - linear regression",
...
somascan_SourceSexAge_adj %>% #1,875,960 rows
  ...
  labs(title = "Significant plasma proteins by fold-change: T21 vs. BMI",
...
#
ranksList <- c("lm_T21_Aptamer_BMI_Age_Sex_Sample_source_results")
#
...
# Using Mtorc1 Signaling as example here
plotEnrichment2 (pathway = hallmarks$HALLMARK_MTORC1_SIGNALING,
                 stats = lm_T21_Aptamer_BMI_Age_Sex_Sample_source_results_ranks,
                 res = unweighted_gsea_res_tbl %>% select(-Query),
                 title = "MTORC1 SIGNALING")
#
ggsave(filename = here("plots", paste0(out_file_prefix, "enrichment_Mtorc1 Signaling", ".pdf")), device = cairo_pdf, width = 5, height = 4, units = "in")
