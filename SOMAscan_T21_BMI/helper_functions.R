...
volcano_plot_lab_lm <- function(res, title = "", 
  ...
  res %>% 
    ...
    xlim(-x_lim, x_lim) +
    ylim(y_lim) +
    geom_text_repel(data = res %>% filter(!is.na(BHadj_pval)) %>% filter(BHadj_pval < 0.1 & FoldChange>1) %>% slice_max(order_by = -BHadj_pval, n=10), aes(label = GeneSymbol), min.segment.length = 0, max.overlaps = Inf, show.legend = FALSE, nudge_x = 0.003, nudge_y = 0.05, family = "Arial") +
    geom_text_repel(data = res %>% filter(!is.na(BHadj_pval)) %>% filter(BHadj_pval < 0.1 & FoldChange<1) %>% slice_max(order_by = -BHadj_pval, n=10), aes(label = GeneSymbol), min.segment.length = 0, max.overlaps = Inf, show.legend = FALSE, nudge_x = -0.003, nudge_y = 0.05) +
