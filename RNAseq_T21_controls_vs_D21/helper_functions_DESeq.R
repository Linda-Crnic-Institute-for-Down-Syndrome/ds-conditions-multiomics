...
get_results_tbl <- function(x, contrast = c(predictor, "T21", "Control"), cooks = TRUE, ind_filt = TRUE, shrink_type = "apeglm", norm_counts) {
  # relevel $condition and run nbinomWaldTest() to ensure that coefficient of
  # interest is available (check with resultsNames()) for lfcShrink() -
  # otherwise need to use contrast argument which does not allow use of apeglm
  # shrinkage
  x$Karyotype <- x$Karyotype %>% relevel(contrast[3]) # HARDCODED FOR NOW 
